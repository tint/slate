...
- Attribute diagnostics are opt-in and match element attributes by raw source name, so rules can distinguish `className` from `class`.
- Attribute diagnostics apply to standard HTML elements and custom elements, including JSX intrinsic elements. Props passed to imported `.slate` components are not checked.
