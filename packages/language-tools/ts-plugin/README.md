...
- Common intrinsic tags expose tag-specific attributes, so values like `button type`, image loading, media preload, track kind, and SVG attributes are checked.
- Unknown custom JSX attributes are allowed and emitted literally by the runtime.
